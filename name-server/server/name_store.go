...
func Register(serviceName, address string) {
	...
	if !ok {
		ns.data[serviceName] = make(map[string]*Address)
	}
...
func (ns *nameStore) clearData() {
	...
	for {
		...
		case <-timeTicker.C:
			list := make([]*Address, 0)
			ns.expireLocker.Lock()
			for k, v := range ns.expireAtData {
				if k <= time.Now().Unix()-10 {
					list = append(list, v...)
					delete(ns.expireAtData, k)
				}
			}
			ns.expireLocker.Unlock()
